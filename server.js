...
//Add code to utilize Mongoose for scraping
...
//Begin creating express app

var app = express();
var PORT = process.env.PORT || 3000;

//Set up .use for express app
...
app.get("/", function(req, res) {
    Article.find({}, null, {sort: {created: -1}}, function(err, data) {
        if(data.length === 0) {
            res.render("placeholder", {message: "Nothing here yet!  Please click button above to scrape for new articles"})
        }
        else {
...
app.get("/saved", function(req, res) {
    Article.find({issaved: true}, null, {sort: {created: -1}}, function(err, data) {
        if (data.length === 0) {
            res.render("placeholder", {message: "You haven't saved any articles yet!"})
        }
        else {
...
app.post("/search", function(req, res) {
    ...
    Article.find({$text: {$search: req.body.search, $caseSensitive: false}}, null, {sort: {created: -1}}, function(err, data) {
        console.log(data);
        if (data.length === 0) {
            res.render("placeholder", {message: "No results.  Please try a different search"});
        }
        else {
